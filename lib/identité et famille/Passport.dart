...
import 'Formulaires/DemandePasseport.dart';

class Passeport extends StatelessWidget {
  Future<String?> _getDocumentPath() async {
    final documents =
        await DatabaseHelper.instance.getDocumentsByName('Passport.pdf');
          ...
          children: [
            ...
            ElevatedButton.icon(
              onPressed: () {
                Navigator.push(
                  context,
                  MaterialPageRoute(builder: (context) => DemandeDePasseport()),
                );
              },
              icon: Icon(Icons.send),
              label: Text("REMPLIR EN LIGNE"),
              style: ElevatedButton.styleFrom(
                padding: EdgeInsets.symmetric(vertical: 12.0, horizontal: 20.0),
                textStyle: TextStyle(fontSize: 16),
              ),
